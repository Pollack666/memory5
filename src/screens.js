  ...
  return (
    <View style={{ flex:1 }}>
      ...
      <View style={{ paddingHorizontal:14, marginBottom:10 }}>
        <Card>
          <View style={{ flexDirection:'row', alignItems:'center', gap:12 }}>
            ...
            <View style={{ alignItems:'flex-end' }}>
              <Text style={{ fontSize:9, color:C.text3, fontWeight:'700', textTransform:'uppercase' }}>Your code</Text>
              <Text style={{ fontSize:20, fontWeight:'800', color:C.accent, letterSpacing:2 }}>{user.code}</Text>
            </View>
          </View>
        </Card>
      </View>
      {showAdd&&(
        <View style={{ paddingHorizontal:14, marginBottom:10 }}>
          <Card>
            <Text style={{ fontSize:13, fontWeight:'700', color:C.text, marginBottom:10 }}>Enter friend's 6-char code</Text>
            <View style={{ flexDirection:'row', gap:9 }}>
              <TextInput style={{ ...inputStyle, flex:1, letterSpacing:2, textTransform:'uppercase' }} placeholder="ABC123" placeholderTextColor={C.text3} maxLength={6} value={code} onChangeText={t=>setCode(t.toUpperCase())} autoCapitalize="characters" autoCorrect={false}/>
              <Btn label="Add" variant="primary" onPress={addFriend} style={{ paddingHorizontal:20 }}/>
            </View>
          </Card>
        </View>
      )}
      <View style={{ flexDirection:'row', gap:8, paddingHorizontal:14, marginBottom:10 }}>
        {['friends','shared'].map(t=>(
          <TouchableOpacity key={t} onPress={()=>setTab(t)} activeOpacity={0.75} style={{ paddingHorizontal:14, paddingVertical:6, borderRadius:20, backgroundColor:tab===t?C.accent:C.surface2, borderColor:tab===t?C.accent:C.border, borderWidth:1 }}>
            <Text style={{ fontSize:11, fontWeight:'700', color:tab===t?'#fff':C.text2, textTransform:'capitalize' }}>{t==='friends'?`Friends (${friends.length})`:'Shared events'}</Text>
          </TouchableOpacity>
        ))}
      </View>
      <ScrollView style={{ flex:1 }} contentContainerStyle={{ paddingHorizontal:14, paddingBottom:90 }} showsVerticalScrollIndicator={false}>
        {tab==='friends'&&friends.length===0&&<EmptyState icon="👥" title="No friends yet" sub="Tap '+ Add' and enter a friend's code to connect"/>}
        {tab==='friends'&&friends.map(f=>(
          <Card key={f.id} style={{ marginBottom:9 }}>
            <View style={{ flexDirection:'row', alignItems:'center', gap:12 }}>
              <View style={{ width:40, height:40, borderRadius:12, backgroundColor:C.accent, alignItems:'center', justifyContent:'center' }}>
                <Text style={{ fontSize:18, fontWeight:'800', color:'#fff' }}>{f.avatar}</Text>
              </View>
              <View style={{ flex:1 }}>
              ...
              <View style={{ alignItems:'flex-end' }}>
                <Text style={{ fontSize:10, color:C.text3 }}>code</Text>
                <Text style={{ fontSize:12, fontWeight:'700', color:C.text3, letterSpacing:1 }}>{f.code}</Text>
              </View>
            </View>
          </Card>
        ))}
        {tab==='shared'&&<EmptyState icon="📤" title="No shared events" sub="Turn on sharing when creating an event to share it with friends"/>}
      </ScrollView>
    </View>
  );
};

export const ProfileScreen = ({ user, setUser, events, friends, fireToast, onLogout }) => {
  ...
  const saveProfile = () => {
    if (!name.trim()) return;
    setUser({ ...user, name:name.trim(), avatar:name.trim()[0]?.toUpperCase()||'?' });
    setEditing(false);
    fireToast('✅','Profile updated','Changes saved');
  };
  const SR = ({ icon, iconBg, label, sub, right, onPress, labelColor }) => (
    <TouchableOpacity onPress={onPress} activeOpacity={onPress?0.75:1} style={{ flexDirection:'row', alignItems:'center', justifyContent:'space-between', padding:13, borderBottomWidth:1, borderBottomColor:C.border }}>
      <View style={{ flexDirection:'row', alignItems:'center', gap:11 }}>
        <View style={{ width:32, height:32, borderRadius:9, backgroundColor:iconBg, alignItems:'center', justifyContent:'center' }}><Text style={{ fontSize:15 }}>{icon}</Text></View>
        <View><Text style={{ fontSize:14, fontWeight:'600', color:labelColor||C.text }}>{label}</Text>{sub?<Text style={{ fontSize:11, color:C.text2, marginTop:1 }}>{sub}</Text>:null}</View>
      </View>
      {right}
    </TouchableOpacity>
  );
    ...
    <View style={{ flex:1 }}>
      ...
      <ScrollView style={{ flex:1 }} contentContainerStyle={{ paddingBottom:90 }} showsVerticalScrollIndicator={false}>
        <View style={{ paddingHorizontal:14, marginBottom:14 }}>
          <Card>
            <View style={{ flexDirection:'row', alignItems:'center', gap:13 }}>
              <View style={{ width:54, height:54, borderRadius:16, backgroundColor:C.accent, alignItems:'center', justifyContent:'center' }}>
                <Text style={{ fontSize:24, fontWeight:'800', color:'#fff' }}>{user.avatar}</Text>
              </View>
              <View style={{ flex:1 }}>
                {editing?<TextInput style={{ ...inputStyle, fontSize:15, fontWeight:'700', paddingVertical:6 }} value={name} onChangeText={setName} autoFocus/>:<Text style={{ fontSize:17, fontWeight:'800', color:C.text }}>{user.name}</Text>}
                <Text style={{ fontSize:12, color:C.text2, marginTop:2 }}>@{user.username}</Text>
                <View style={{ flexDirection:'row', gap:14, marginTop:6 }}>
                  <Text style={{ fontSize:11, color:C.text3 }}><Text style={{ fontWeight:'700', color:C.text }}>{events.length}</Text> events</Text>
                  <Text style={{ fontSize:11, color:C.text3 }}><Text style={{ fontWeight:'700', color:C.text }}>{friends.length}</Text> friends</Text>
                </View>
              </View>
              <View style={{ alignItems:'flex-end' }}>
                <Text style={{ fontSize:9, color:C.text3, fontWeight:'700', textTransform:'uppercase' }}>Friend code</Text>
                <Text style={{ fontSize:18, fontWeight:'800', color:C.accent, letterSpacing:2 }}>{user.code}</Text>
              </View>
            </View>
            <View style={{ flexDirection:'row', gap:9, marginTop:13 }}>
              {editing?<><Btn label="Save" variant="primary" style={{ flex:1 }} onPress={saveProfile}/><Btn label="Cancel" variant="ghost" style={{ flex:1 }} onPress={()=>{ setEditing(false); setName(user.name); }}/></>:<Btn label="✏️  Edit profile" variant="ghost" style={{ flex:1 }} onPress={()=>setEditing(true)}/>}
            </View>
          </Card>
        </View>
        <SectionLabel>Notifications</SectionLabel>
        <View style={{ marginHorizontal:14, backgroundColor:C.card, borderColor:C.border, borderWidth:1, borderRadius:18, overflow:'hidden', marginBottom:14 }}>
          <SR icon="🔔" iconBg="rgba(245,166,35,0.15)" label="Push notifications" sub="Lock screen & Dynamic Island" right={<Switch value={notifs} onValueChange={setNotifs} trackColor={{ false:C.surface2, true:C.accent }} thumbColor="#fff"/>}/>
          <SR icon="🔊" iconBg="rgba(62,207,142,0.15)" label="Sound" right={<Switch value={sound} onValueChange={setSound} trackColor={{ false:C.surface2, true:C.accent }} thumbColor="#fff"/>}/>
          <SR icon="📳" iconBg="rgba(79,115,255,0.15)" label="Vibration" right={<Switch value={true} onValueChange={()=>{}} trackColor={{ false:C.surface2, true:C.accent }} thumbColor="#fff"/>}/>
        </View>
        <SectionLabel>Location</SectionLabel>
        <View style={{ marginHorizontal:14, backgroundColor:C.card, borderColor:C.border, borderWidth:1, borderRadius:18, overflow:'hidden', marginBottom:14 }}>
          <SR icon="📍" iconBg="rgba(255,95,95,0.15)"   label="Background location" sub="Required for proximity alerts" right={<Switch value={bgLoc} onValueChange={setBgLoc} trackColor={{ false:C.surface2, true:C.accent }} thumbColor="#fff"/>}/>
          <SR icon="⚡" iconBg="rgba(62,207,142,0.15)"  label="Battery saver"       sub="Less frequent GPS checks"    right={<Switch value={battery} onValueChange={setBattery} trackColor={{ false:C.surface2, true:C.accent }} thumbColor="#fff"/>}/>
        </View>
        <SectionLabel>Default alert radius</SectionLabel>
        <View style={{ marginHorizontal:14, marginBottom:14 }}>
          <Card>
            <View style={{ flexDirection:'row', alignItems:'baseline', gap:4, marginBottom:12 }}>
              <Text style={{ fontSize:24, fontWeight:'800', color:C.accent }}>{defRadius}</Text>
              <Text style={{ fontSize:13, color:C.text2 }}>metres</Text>
            </View>
            <View style={{ flexDirection:'row', justifyContent:'space-between' }}>
              {[50,100,150,200,300,500].map(v=>(
                <TouchableOpacity key={v} onPress={()=>setDefRadius(v)} activeOpacity={0.75} style={{ paddingHorizontal:9, paddingVertical:6, borderRadius:8, backgroundColor:defRadius===v?C.accent:C.surface2, borderColor:defRadius===v?C.accent:C.border, borderWidth:1 }}>
                  <Text style={{ fontSize:11, fontWeight:'700', color:defRadius===v?'#fff':C.text2 }}>{v}m</Text>
                </TouchableOpacity>
              ))}
            </View>
          </Card>
        ...
        <View style={{ marginHorizontal:14, backgroundColor:C.card, borderColor:C.border, borderWidth:1, borderRadius:18, overflow:'hidden', marginBottom:14 }}>
          <SR icon="☁️" iconBg="rgba(79,115,255,0.15)"  label="iCloud sync"      sub="Sync across your devices" onPress={()=>fireToast('☁️','iCloud','Opens iOS iCloud settings')}    right={<Text style={{ color:C.text3 }}>›</Text>}/>
          <SR icon="🔒" iconBg="rgba(176,111,255,0.15)" label="Privacy"           sub="Location, notifications"  onPress={()=>fireToast('🔒','Privacy','Opens iOS Privacy settings')} right={<Text style={{ color:C.text3 }}>›</Text>}/>
          <SR icon="🗑️" iconBg="rgba(255,95,95,0.15)"  label="Clear all events"  labelColor={C.danger}
            onPress={()=>Alert.alert('Clear all events?',`Delete all ${events.length} events. Cannot be undone.`,[
              { text:'Cancel', style:'cancel' },
              { text:'Clear', style:'destructive', onPress:async()=>{ await Storage.remove('events'); fireToast('🗑️','Cleared','All events removed'); } },
            ])} right={<Text style={{ color:C.text3 }}>›</Text>}/>
          <SR icon="🚪" iconBg="rgba(255,95,95,0.12)"  label="Sign out" labelColor={C.danger}
            onPress={()=>Alert.alert('Sign out?','You will need to create a new profile.',[
              { text:'Cancel', style:'cancel' },
              { text:'Sign out', style:'destructive', onPress:onLogout },
            ])} right={<Text style={{ color:C.text3 }}>›</Text>}/>
        </View>
        <Text style={{ textAlign:'center', fontSize:11, color:C.text3, paddingBottom:20 }}>Memory 5 · v1.0{'\n'}Your data stays on your device 🔒</Text>
      </ScrollView>
    </View>
  );
};
